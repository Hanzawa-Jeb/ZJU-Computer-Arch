...
- Hardware Parallelism
	...
	- Vector Architecture & GPUs & Multimedia ISA(DLP)

ILP
- 初始的是pipelining
- 还有speculative exec(例如分支预测)
	- 就是提前完成一部分的工作
	- 最经典的就是Branch Prediction
	- 假如预测有错，就进行flush即可
- Superscalar(多发射)也属于ILP的一种
Vector Arch
- 支持一个Vector Instruction进行多重数据的处理
Classes of Parallel Arch (Flynn分类法)
- SISD/SIMD/MISD/MIMD
- S/M: Single/Multiple, I/D: Instruction/Data
- SISD: 单指令单数据
- SIMD: 单指令多数据，同一个指令被Multiple Processors处理，使用Different Data Streams
- MIMD: 现代多核处理器最常见
