...
Since the instructions that don't use rs1 are rare, so we assign rs1use as `!(LUI | AUIPC | JAL)`. For the rs2use, we assign it `(R_valid | S_valid | B_valid)` since these three types of instructions use rs2.
...
Then I discover that the update process here depends on the `reg_flush` signals, and the signal `reg_EM_flush` was not initialized and not assigned, so the `hazard_optype_MEM` can't be correctly updateted, making it unable to get the value of `hazard_optype_MEM`, so I initialized it with `1'b0`, and the problem was solved.
